...
(function(){const all=Array.isArray(window.LISTINGS_DATA)?window.LISTINGS_DATA:[];const current=all.find(x=>x.id==='104-paul-avenue');const pill=document.getElementById('property-status-pill');if(current&&pill){pill.textContent=current.status;pill.classList.add('status-'+current.status.toLowerCase().replace(/[^a-z0-9]+/g,'-'));}const target=document.getElementById('related-listings');if(!target)return;const related=all.filter(x=>x.id!=='104-paul-avenue'&&x.status!=='Sold').slice(0,3);target.innerHTML=related.map(x=>`<article><div style="background-image:url('${x.image}')"><span>${x.status}</span></div><h3>${x.address}</h3><p>${x.price} · ${x.cityStateZip}</p><a href="${x.page}">${x.placeholder?'Details coming soon':'Explore property'} →</a></article>`).join('');})();
